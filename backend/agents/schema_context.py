...
EDGE_LABELS = {
    "order_to_delivery": "Order -> Delivery (via order_ids)",
    "delivery_to_invoice": "Delivery -> Invoice (via delivery_ids)",
    "invoice_to_payment": "Invoice -> Payment (via clearing links)",
    "invoice_to_journal_entry": "Invoice -> JournalEntry (via referenceDocument/accountingDocument)",
    "order_to_customer": "Order -> Customer (via customer_id)",
    "order_to_product": "Order -> Product (via sales order items)",
    "delivery_to_product": "Delivery -> Product (via outbound delivery items)",
    "customer_to_address": "Customer -> Address (via address_ids)",
    "customer_to_delivery": "Customer -> Delivery (derived from customer orders)",
    "delivery_to_plant": "Delivery -> Plant (via delivery item plant)",
    "product_to_plant": "Product -> Plant (via product_plants)",
}
...
def get_schema_dict(builder: GraphBuilder | None = None) -> dict[str, Any]:
    ...
    for _, attrs in graph.nodes(data=True):
        ...
        fields_by_type[entity_type].update(key for key in attrs.keys() if key not in {"entity_type", "label", "node_key"})
...
def get_schema_prompt(builder: GraphBuilder | None = None) -> str:
    ...
    lines.extend(["", "Dataset note: The provided SAP data contains Sales Orders, not Purchase Orders. Order -> Product models the SalesOrderItem -> Material relationship available in the source data."])
    return "\n".join(lines)
